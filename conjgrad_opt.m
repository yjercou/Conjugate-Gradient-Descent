...
function [xopt,fopt,niter,gnorm,dx] = conjgrad_opt(varargin)
...
while and(gnorm>=tol, and(niter <= maxiter, dx >= dxmin))
    ...
    elseif(i > 0)
        gnew = grad(x);
        gold = grad(xold);
        alphak = norm(gnew,2)/norm(gold,2);
        pk1 = -gnew + alphak*pk;
        
        if(pk1'*gnew <= 0)
            [tk1,~] = fminbnd(@(tk) iterbyConjugate(tk,x,pk1),0,1e-1);
            xnew = x + tk1  *  pk1;
            pk = pk1;
        else
            i = 0;
...
function frosen = iterbyConjugate(tk,A,B)
    xnew = A + tk*B;
    frosen = Rosenbrock(xnew(1),xnew(2));
end
